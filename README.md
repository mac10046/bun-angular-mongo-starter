...
A production-ready, full-stack authentication system built with Bun/Express backend and Angular frontend. This boilerplate provides a complete authentication solution including local authentication, Google OAuth, email verification, password management, and a comprehensive admin panel with RBAC (Role-Based Access Control).
...
4. **Run setup**: `bun run setup && bun run dev`
...
- **One-Command Setup**: `bun run setup && bun run dev` starts everything
...
- **Fast Runtime**: Powered by Bun for faster execution and package management
...
### Prerequisites
- Bun 1.0+ (Install: `curl -fsSL https://bun.sh/install | bash` or on Windows: `powershell -c "irm bun.sh/install.ps1 | iex"`)
...
bun run dev:frontend  # http://localhost:4200
bun run dev:admin     # http://localhost:4300
```
...
```bash
bun scripts/db-setup.js --with-admin --admin-email=admin@example.com --admin-password=YourSecurePassword123
```
...
| `bun run start:prod` | Start production with PM2 |
| `bun run stop:prod` | Stop PM2 processes |
| `bun run logs` | View PM2 logs |
| `bun run db:setup` | Initialize database with collections and roles |
| `bun run db:setup:admin` | Initialize database and create admin user |

### Backend Scripts
| Script | Description |
|--------|-------------|
| `bun run dev` | Start with Bun's built-in watch mode |
...
Built with Bun, Express, MongoDB, Angular, and Bootstrap.
