...
userSchema.pre('save', async function (next) {
  if (this.isModified('role')) {
    ...
    const roleExists = await Role.findOne({ name: this.role });
    if (!roleExists) {
      const error = new Error(`Invalid role: ${this.role}. Role must be seeded first.`);
...
/**
 * Get user data with role info and permissions
 * @returns {Promise<Object>} User data with permissions
 */
userSchema.methods.toJSONWithPermissions = async function () {
  const Role = mongoose.model('Role');
  const user = this.toObject();
  delete user.password;
  delete user.__v;
