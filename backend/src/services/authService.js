...
class AuthService {
  ...
  async login(email, password) {
    ...
    const userWithPermissions = await this.sanitizeUserWithPermissions(user);
  ...
  async googleAuth(googleUser) {
    ...
    // Get user with permissions
    const userWithPermissions = await this.sanitizeUserWithPermissions(googleUser);
   ...
   * @param {boolean} includePermissions - Whether to include permissions
   * @returns {Promise<Object>} User object
   */
  async getUserById(userId, includePermissions = false) {
    ...
    if (includePermissions) {
      return this.sanitizeUserWithPermissions(user);
    }
   ...
   * @param {boolean} options.includePermissions - Include user permissions
  ...
  sanitizeUser(user, options = {}) {
    ...
    // Include permissions if provided (from async call)
    if (options.permissions) {
      sanitized.permissions = options.permissions;
    }

    // Include role info if provided
    if (options.roleInfo) {
      sanitized.roleInfo = options.roleInfo;
    }
  ...
  /**
   * Sanitize user with permissions included
   * @param {Object} user - User document (must be a Mongoose document with methods)
   * @returns {Promise<Object>} Sanitized user with permissions
   */
  async sanitizeUserWithPermissions(user) {
    ...
    return this.sanitizeUser(user, { permissions, roleInfo });
  }
}
